"""Forrester transfer learning benchmark with negative shift -0.15 and noise 0.5.
...
• Source: Left-shifted Forrester (shift=-0.15) with noise_std=0.5
...
def forrester_neg_shift_015_noise_05(
    settings: ConvergenceBenchmarkSettings,
) -> pd.DataFrame:
    """Forrester TL benchmark with negative shift -0.15 and noise level 0.5.

    This benchmark evaluates transfer learning from spatially shifted source
    domains. Tests TL model ability to handle domain shift where the source
    function is evaluated at g(x) = f(x + 0.15), creating misalignment between
    source and target input-output relationships.
...
# Benchmark instance
forrester_neg_shift_015_noise_05_benchmark = ConvergenceBenchmark(
    function=forrester_neg_shift_015_noise_05,
