"""Forrester transfer learning benchmark with negative shift -0.1 and noise 0.5.
...
• Source: Left-shifted Forrester (shift=-0.1) with noise_std=0.5
...
def forrester_neg_shift_01_noise_05(
    settings: ConvergenceBenchmarkSettings,
) -> pd.DataFrame:
    """Forrester TL benchmark with negative shift -0.1 and noise level 0.5.

    This benchmark evaluates transfer learning from moderately shifted source
    domains. Tests TL model ability to handle smaller domain shifts where
    g(x) = f(x + 0.1), examining sensitivity to shift magnitude.
...
# Benchmark instance
forrester_neg_shift_01_noise_05_benchmark = ConvergenceBenchmark(
    function=forrester_neg_shift_01_noise_05,
