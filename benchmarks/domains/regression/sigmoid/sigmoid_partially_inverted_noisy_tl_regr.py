"""TL regression benchmark for sigmoid function partially inverted with noise.

Target task: sin(x) + noise(0, 0.5) for x ∈ [0,π] ∪ [π,2π]  
...
def sigmoid_partially_inverted_noisy_tl_regr(
    ...
    This benchmark uses partially inverted sine functions with noise as tasks:
    ...
    • Different noise levels for target vs source
    • Noisy sine function (minimization objective)

    Key characteristics:
    • Parameters:
      - x: Numerical discrete (17 points from 0 to 2π)
      - Function: Task parameter (Test, Training)
    • Target: Noisy sine function output (continuous)
    • Objective: Minimization (approximate minimum considering noise)
...
benchmark_config = TransferLearningRegressionBenchmarkSettings(
    ...
    noise_std=0.0,  # Noise is already built into functions
)

# Create the benchmark
sigmoid_partially_inverted_noisy_tl_regr_benchmark = TransferLearningRegressionBenchmark(
    function=sigmoid_partially_inverted_noisy_tl_regr, 
